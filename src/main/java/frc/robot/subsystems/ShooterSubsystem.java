...
public class ShooterSubsystem extends SubsystemBase {
  ...
  public final ProfiledPIDController turretPID =
      new ProfiledPIDController(
          3.1856, 0, 1.13513, new TrapezoidProfile.Constraints(Math.PI / 2, Math.PI / 2)); //

  private final NetworkTableEntry shooterAdjustmentNTE;
  private final LimelightDataLatch distanceLatch =
      new LimelightDataLatch(LimelightDataType.DISTANCE, 5);
  ...
  public void periodic() {
    ...
    if (bangBangRunning) {
      ...
      if (Math.abs(bangBangOffset) >= currentShooterToleranceDegrees) {
        turnTurret(
            Math.signum(bangBangOffset) * slowTurretTurnSpeed + turretTurnSpeed / bangBangOffset);
      } else {
        stopTurret();
      }
    }
