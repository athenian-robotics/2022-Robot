...
// A LimelightDataLatch allows a command to put in a request for Limelight data, check if it has
// arrived, and retrieve
// it when it's ready.
// Throws a GoalNotFoundException when it times out without receiving data. timeoutSeconds is
// customizable but has a
// default value of a quarter second.
// Unlocked DataLatches (unlocked() == true) MUST BE RESET AND RESCHEDULED to retrieve new data. The
// simplest way is
// to call limelightSubsystem.addLatch(YOUR_LATCH.reset());
// I've left open() free of GoalNotFoundException because I trust you to always check unlocked() ==
// true before
// opening the latch
