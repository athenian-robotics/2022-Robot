...
// Adds the ability to sidestep within an order-sensitive command execution scenario by temporarily
// replacing the
// current command with another.
// Calling the detour() method will immediately start executing the secondary command and the
// original won't start
...
// A DetourableCommand ends when the default Command ends while executing or when the secondary
// command ends and
// endAfterDetour is set to true.
// Classes that extend DetourableCommand will need to implement the standard Command methods with an
// underscore before
