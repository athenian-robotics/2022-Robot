...
import frc.robot.commands.outtake.AlwaysTurretTurnToGoalWithLimelightAndSetHoodAngleOrManualControl;
import frc.robot.commands.outtake.ShootBalls;
...
public class RobotContainer {
    ...
    public RobotContainer() {
        ...
        outtake.setDefaultCommand(new AlwaysTurretTurnToGoalWithLimelightAndSetHoodAngleOrManualControl(limelight,
                outtake, shooterDataTable)); // Check fight stick y-axis for
        SmartDashboard.putData("AutoChooser", chooser);
        chooser.setDefaultOption("0: 2.5 Meters Forward", new AutoRoutine0(drivetrain));
        chooser.addOption("1: 5 Ball Auto - Bottom Left Start", new AutoRoutine1(climb, drivetrain, indexer, intake,
                outtake, limelight, shooterDataTable));
        chooser.addOption("2: 2 Ball Auto - Top Left Start", new AutoRoutine2(climb, drivetrain, indexer, intake,
                outtake, limelight, shooterDataTable));
        chooser.addOption("3: 2 Ball Auto - Bottom Left Start", new AutoRoutine3(climb, drivetrain, indexer, intake,
                outtake, limelight, shooterDataTable));
        chooser.addOption("4: 4 Ball Auto - Bottom Left Start", new AutoRoutine4(climb, drivetrain, indexer, intake,
                outtake, limelight, shooterDataTable));
        chooser.addOption("5: 2 Ball Auto", new AutoRoutine5(climb, drivetrain, indexer, intake, outtake, limelight,
                shooterDataTable));
        xboxButtonSetup();
    ...
    private void configureButtonBindings() {
        ...
        FightStick.fightStickX.whenPressed(new ShootBalls(climb, drivetrain, indexer, intake, outtake, limelight,
                shooterDataTable));
        FightStick.fightStickA.whenPressed(new ToggleIntake(intake)); // Toggle intake wheels and pneumatics
        FightStick.fightStickY.whenPressed(new ShootTwo(climb, drivetrain, indexer, intake, outtake, limelight,
                shooterDataTable, xboxController));
