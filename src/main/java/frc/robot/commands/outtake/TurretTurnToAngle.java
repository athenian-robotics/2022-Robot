...
public class TurretTurnToAngle extends CommandBase {
    ...
    @Override
    public void execute() {
        if (angle < minimumTurretAngleRadians && angle - shooterSubsystem.getTurretAngleRadians() < 0 || angle > maximumTurretAngleRadians && angle - shooterSubsystem.getTurretAngleRadians() > 0)
            System.out.println("Error! angle passed into TurretTurnToAngle " + this + " was out of bounds");
    }

    @Override
    public boolean isFinished() {
        return Math.abs(shooterSubsystem.getTurretAngleRadians()-Math.toRadians(angle))<=0.075; //0.5 position tolerance
    }
