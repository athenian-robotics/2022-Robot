...
public class AlwaysTurretTurnToGoalWithOdometryAndSetHoodAngleOrManualControl extends CommandBase {
    ...
    public AlwaysTurretTurnToGoalWithOdometryAndSetHoodAngleOrManualControl(DrivetrainSubsystem drivetrainSubsystem, LimelightSubsystem limelightSubsystem, ShooterSubsystem shooterSubsystem, ShooterDataTable shooterDataTable) {
    ...
    public void execute() {
        if (FightStick.fightStickJoystick.getX() < -0.5) { //TURRET ADJUSTMENT FALCON
            ...
            shooterSubsystem.setTurretPositionRadians(Math.toRadians(drivetrainSubsystem.getGyroAngle()) - Math.atan2(goalVector.getY(), goalVector.getX()));
        } else {
