...
public class AutoTurnAngle extends CommandBase {
  ...
  public void initialize() {
    setpoint =
        drivetrainSubsystem.getHeading() + angleToTurn; // Configure pid tolerance and setpoint
  ...
  public void execute() {
    power =
        Math.min(
            Math.abs(pid.calculate(drivetrainSubsystem.getGyroYaw())),
            Constants.AutoConstants.maxAutoTurn); // Scale under max constant
    int leftSign =
        Math.abs(setpoint - drivetrainSubsystem.getGyroYaw()) > 180 ? -1 : 1; // Fastest direction
    int rightSign =
        Math.abs(setpoint - drivetrainSubsystem.getGyroYaw()) > 180 ? 1 : -1; // Fastest direction
    power = Math.abs(power) < 0.01 ? 0 : power; // Deadband
    System.out.println(power); // Debugging purposes
    drivetrainSubsystem.autoTankDrive(
        power * leftSign, power * rightSign); // Drive with calculated power
  }
