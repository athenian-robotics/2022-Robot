...
public class AutoForwardDistance extends CommandBase {
  ...
  public void initialize() {
    double setpoint =
        drivetrain.getRightDistanceDriven() + metersToDrive; // Configure PID setpoint and tolerance
  ...
  public void execute() {
    ...
    double drivePower =
        Math.min(Math.abs(pid.calculate(drivetrain.getRightDistanceDriven())), maxAutoSpeed); //
